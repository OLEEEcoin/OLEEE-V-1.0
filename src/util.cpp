...
const char * const BITCOIN_CONF_FILENAME = "oleeecoin.conf";
const char * const BITCOIN_PID_FILENAME = "oleeecoin.pid";
...
#else
    const char* pszModule = "oleeecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Oleeecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Oleeecoin";
#else
    // Unix
    return pathRet / ".oleeecoin";
#endif
#endif
}
